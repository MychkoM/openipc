...
| `nightMode.lightSensorPin` | **not set / 0** | – | no GPIO ambient-light sensor present (software monitor only) |
...
* There is **no GPIO connection from the backlight board to the SoC**: the
...
* Day/night is controlled by the software light monitor (`lightMonitor: true`,
  thresholds `minThreshold: 15000`, `maxThreshold: 30000`).
